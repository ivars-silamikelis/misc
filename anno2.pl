...
my $gene_gb="/home/ivars/Data/Mtb/references/MTuber_ref_diff_strains/reannotate/CTRI-2.gbk";
...
for my $feat_obj ($seq->get_SeqFeatures()){
  if ($feat_obj->primary_tag eq "CDS"){
    if ($feat_obj->has_tag('gene')){
      ...
      my @coords=&extr_cds_coords($feat_obj->spliced_seq->seq,$feat_obj->get_tag_values('gene'));
      unless($coords[0]==0){
        for my $kref (@coords){
          my $feat=&create_tag($kref,$feat_obj->get_tag_values('gene'));
          $gb_seq->add_SeqFeature($feat);
          print "Adding ", $feat_obj->get_tag_values('gene'),"\n";
        }
      }
    }
  }
